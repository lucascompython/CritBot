...
class Tracks(Enum):
    YoutubeTrack = "youtube" | "yt"
    SpotifyTrack = "spotify" | "sp"
    SoundCloudTrack = "soundclou" | "sc"
    YoutubeMusicTrack = "youtube_music" | "ytm"
    YoutubePlaylist = "youtube_playlist" | "ytpl"
...
class Music(commands.Cog):
    ...
    async def get_dislikes(self, track_id: str) -> int | str | None:
        ...
        async with self.bot.web_client.get(
            f"https://returnyoutubedislikeapi.com/votes?videoId={track_id}"
        ) as resp:
            try:
    ...
    async def embed_generator(
        self, track: wavelink.YouTubeTrack, author: discord.Member
    ) -> discord.Embed:
        ...
        partial = functools.partial(
            self.ytdl.extract_info, track.identifier, download=False, process=False
        )
        ...
        if subs:
            subs = self.human_format(subs)
        if views:
            views = self.human_format(views)
        if likes:
            likes = self.human_format(likes)
        if type(dislikes) != str:
            dislikes = self.human_format(dislikes)
        ...
        embed = discord.Embed(
            title=self.t(
                "embed", "title", source=track.info["sourceName"].capitalize()
            ),
    ...
    async def _add_to_queue(
        self,
        tracks: wavelink.YouTubeTrack | wavelink.YouTubePlaylist,
        player: wavelink.Player,
        ctx: commands.Context,
        playlist: Optional[wavelink.YouTubePlaylist] = None,
    ) -> None:
        self.bot.i18n.command_name = "play"
        if playlist:
            for track in tracks:
            ...
            return await ctx.send(
                self.t(
                    "cmd", "queued_playlist", length=len(tracks), playlist=playlist.name
                )
            )
    ...
    async def play(
        ...
        *,
        ...
        track = await wavelink.GenericTrack.search(query, return_first=True)
        print(track)
        ...
        ):  # if it is actually a spotify link the function ends in this if statement
            ...
            match type:
                case spotify.SpotifySearchType.track:
                    ...
                    if vc.is_playing():
                        return await ctx.send(
                            self.t(
                                "cmd", "queued", track=track.title, author=track.author
                            )
                        )
                ...
                case spotify.SpotifySearchType.album | spotify.SpotifySearchType.playlist:
                    first_track = True
                    iterator = spotify.SpotifyTrack.iterator(
                        query=id, type=type, partial_tracks=True
                    )
                    async for track in iterator:
                        ...
                        if first_track and not vc.is_playing():
                            ...
                            await ctx.send(
                                embed=await self.embed_generator(track, ctx.author)
                            )
                            first_track = False

                    count = (
                        iterator._count if not vc.is_playing() else iterator._count - 1
                    )

                    return await ctx.send(
                        self.t("cmd", "queued_spotify_playlist", length=count)
                    )
        ...
        if not vc.is_playing():
            ...
            if not is_playlist:
                ...
                await vc.queue.put_wait(track)
                return await ctx.send(
                    self.t("cmd", "queued", track=track.title, author=track.author)
                )
    ...
    async def history(self, ctx: commands.Context) -> None:
        ...
        try:
            embed = discord.Embed(
                description="\n".join(
                    [
                        f'[{i+1}]({track.uri}) -- `{track.title}` {self.t("embed", "by")} `{track.info["context"].author}` {self.t("embed", "at")} {discord.utils.format_dt(track.info["time"], "t")}'
                        for i, track in enumerate(reversed(vc.queue.history))
                    ]
                )
            )
            embed.set_author(
                name=self.t("embed", "title", user=ctx.author.name),
    ...
    async def on_wavelink_track_end(self, payload: wavelink.TrackEventPayload):
        ...
        if payload.player.queue.history[-1].info[
            "loop"
        ]:  # if the last song is in loop, play it again
            track.info["loop"] = True
            track.info["time"] = datetime.datetime.now()
            return await payload.player.play(track)
        ...
        payload.player.queue.history[-1].info["time"] = datetime.datetime.now()
        ctx = payload.player.queue.history[-1].info["context"]
        requester = ctx.author
        track = await payload.player.play(track)
        embed = await self.embed_generator(track, requester)
        setattr(payload.player.queue.history[-1], "uri", track.uri)
        await ctx.send(embed=embed)

        payload.player.track.info["context"] = ctx
        payload.player.track.info["loop"] = False
        payload.player.queue.history[-1].info["context"] = ctx
        payload.player.queue.history[-1].info["loop"] = False
    ...
    # TODO see the "comesfromplaycommand" thing, this is horrible
        ...
        if not ctx.voice_client and not channel:
            ...
            await ctx.send(
                self.t(
                    "cmd",
                    "connect",
                    channel=ctx.author.voice.channel.mention,
                    mcommand_name="join",
                )
            )
    ...
    def _estimate_time_until(
        self, track: wavelink.tracks.Playable, player: wavelink.Player
    ) -> str:
        ...
        estimated_time = sum(
            track.length if hasattr(track, "length") else 0
            for track in queue_until_track
        )  # PartialTracks don't have a length attribute
        ...
        if len(vc.queue._queue) == 0:
            embed = discord.Embed(
                description=self.t(
                    "embed",
                    "description",
                    track=vc.track.title,
                    user=vc.track.info["context"].author,
                    current_time=self.parse_duration(progress),
                    total_time=self.parse_duration(vc.track.duration),
                )
            )
            embed.set_author(
            ...
            if j < 9:
                if hasattr(track, "duration"):
                    embed.add_field(
                        ...
                        value=f"{self.parse_duration(track.duration)} / Est. {self._estimate_time_until(track, vc)}",
                        inline=False,
                    )
                else:
                    embed.add_field(
                        name=f"{i+1}. {track.title}", value=".", inline=False
                    )
            else:
                if hasattr(track, "duration"):
                    embed.add_field(
    ...
    async def now_playing(self, ctx: commands.Context) -> None:
        ...
        embed.add_field(
            ...
            value=f"[{track.title}]({track.uri}) - *({self.parse_duration(track.duration)})*",
            inline=False,
        )
        embed.add_field(
            name=self.t("embed", "by"),
            value=f"[{track.author}]({track.info['uploader_url']})",
            inline=False,
        )
        embed.add_field(
            name="Likes",
            value=f"<:likeemoji:1074046790237700097> {track.info['likes']}",
        )
        embed.add_field(
            name="Dislikes",
            value=f"<:dislikeemoji:1074052963649200198> {track.info['dislikes']}",
        )
        embed.add_field(
            name=self.t("embed", "views"),
            value=f"<:views:1074047661759528990> {track.info['views']}",
        )
        embed.add_field(
            name=self.t("embed", "subs"), value=f':envelope: {track.info["subs"]}'
        )
        embed.add_field(
            name=self.t("embed", "uploaded"),
            value=f":calendar_spiral: {track.info['upload_date']}",
            inline=False,
        )
        embed.add_field(
            name=self.t("embed", "requested_by"),
            value=f"{track.info['context'].author.mention} {self.t('embed', 'ago', time=(datetime.datetime.now() - track.info['time']).seconds)}",
            inline=False,
        )
        embed.add_field(
        ...
        embed.add_field(
            ...
            value=f":track_next: `{vc.queue[0].info['title']} - {self.parse_duration(vc.queue[0].duration)}`"
            if vc.queue
    ...
    async def boost(self, ctx: commands.Context) -> None:
        ...
        await vc.set_filter(
            wavelink.Filter(equalizer=wavelink.Equalizer.boost()), seek=True
        )
        self.filter = True
    ...
    async def lyrics(self, ctx: commands.Context, *, song: str = None):
        ...
        if (
            len(lyrics) > 1820
        ):  # 1820 = 2000 (discord's limit) - 180 (aprox. length of the rest of the message)
            length = 0
    ...
    def _get_filtered_song(song: str) -> str:
        ...
        song = re.sub(
            "[\(\[].*?[\)\]]", "", song
        )  # remove everything between brackets and parenthesis including them
        return song
